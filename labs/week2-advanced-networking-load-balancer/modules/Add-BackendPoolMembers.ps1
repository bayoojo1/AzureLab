function Add-BackendPoolMembers {
    ...
    Write-Host "Adding VMs to backend pool..." -ForegroundColor Cyan
    ...
    $nics = Get-AzNetworkInterface -ResourceGroupName $Config.ResourceGroup | 
        Where-Object { $_.VirtualMachine -and ($Config.WebVms -contains ($_.VirtualMachine.Id.Split('/')[-1])) }
    
    foreach ($nic in $nics) {
        $nic.IpConfigurations[0].LoadBalancerBackendAddressPools = @($backendPool)
        $nic | Set-AzNetworkInterface | Out-Null
        Write-Host "Added $($nic.VirtualMachine.Id.Split('/')[-1]) to backend pool" -ForegroundColor Green
    }
}